...
function createLocalRouter({ db }) {
  ...
  router.post('/likes', (req, res) => {
    ...
    // Match by either id or spotify_id
    ...
    if (index === -1) {
      ...
      db.save(data);
      res.json({ liked: false, trackId: track.id || track.spotify_id });
    }
  });
  ...
  router.post('/likes/sync', (req, res) => {
    ...
    data.likes.forEach((t) => {
      if (t.id == id && !t.spotify_id) {
        t.spotify_id = spotify_id;
        updated = true;
      } else if (t.spotify_id == spotify_id && !t.id) {
        t.id = id;
        updated = true;
      }
    });
  ...
  router.get('/player-state', (req, res) => {
    const data = db.load();
    res.json(data.playerState || {});
  });

  router.post('/player-state', (req, res) => {
