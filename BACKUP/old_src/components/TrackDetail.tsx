...
import { ArrowLeft, Play, ExternalLink, Clock, Music, Disc3, Mic2 } from 'lucide-react';
import {
    fetchTrack,
    fetchAudioFeatures,
    fetchArtist,
...
const MiniTrackCard = ({ track, token, onNavigate }: { track: any; token: string; onNavigate: (id: string) => void }) => {
    ...
    const playTrack = async (e: React.MouseEvent) => {
        e.stopPropagation();
        await playTracks(token, [track.uri]);
    };
...
export const TrackDetail = ({ token, trackId, onNavigateArtist }: TrackDetailProps) => {
                ...
                <div className="track-detail__hero-inner">
                    ...
                    >
                        ...
                        <div className="track-detail__actions">
                            ...
                            <LikeButton trackId={track.id} token={token} size={22} />
                            <motion.button
                                    ...
                                    <div className="mini-tracks-list">
                                        {artistTracks.filter((t: any) => t.id !== trackId).map((t: any) => (
                                            <MiniTrackCard
                                                ...
                                                track={t}
                                                token={token}
                                                onNavigate={(id) => onNavigateArtist?.(id)}
                                            />
                                        ))}
                                    </div>
                                </section>
