...
import {
    ...
    Mic2, ListMusic, MonitorSpeaker, X, Search,
    ...
    return (
        <header className="topbar">
            ...
            <nav className="topbar__nav">
                {['home', 'browse', 'radio', 'playlists', 'activity'].map(page => (
                    <a
                        key={page}
                        className={`topbar__nav-link ${currentPage === page ? 'topbar__nav-link--active' : ''}`}
                        onClick={() => handleNavClick(page)}
                    >
                        {{ home: 'Accueil', browse: 'Parcourir', radio: 'Radio', playlists: 'Playlists', activity: 'Activité' }[page]}
                    </a>
                ))}
            </nav>
            ...
            <div className="topbar__profile-container" ref={menuRef}>
                <div
                    className={`topbar__profile ${menuOpen ? 'topbar__profile--active' : ''}`}
                ...
                <AnimatePresence>
                    {menuOpen && (
                        ...
                        >
                            <div className="profile-menu__item" onClick={() => handleNavClick('profile')}>
                                <User size={16} /><span>Profil</span>
                            </div>
                            <div className="profile-menu__item" onClick={() => handleNavClick('settings')}>
                                <Settings size={16} /><span>Paramètres</span>
                            </div>
                            <div className="profile-menu__divider" />
                            <div className="profile-menu__item profile-menu__item--danger" onClick={onLogout}>
    ...
    return (
        <div className="player-panel">
            ...
            <div className="panel-scroll">
                ...
                {items.length === 0 && <div className="panel-empty-text">File vide</div>}
...
const LyricsPanel = ({ token, track, progressMs }: { token: string; track: any; progressMs: number }) => {
    ...
    if (loading) return (
        <div className="player-panel">
            ...
            <div className="panel-empty-text">Chargement…</div>
        </div>
    );
...
const DevicesPanel = ({ token, activeDeviceId, onTransfer }: {
    ...
    return (
        <div className="player-panel">
            ...
            <div className="panel-scroll">
                {devices.length === 0 && (
                    <div className="panel-empty-text">Aucun appareil actif</div>
                )}
...
export const Player = ({ token }: { token: string | null }) => {
    const {
        playbackState, progressMs, volume, isSeeking, isSaved,
        fetchState, handlePlayPause, handleNext, handlePrevious,
        handleSeek, handleVolume, handleShuffle, handleRepeat, handleLike,
        setProgressMs, setIsSeeking, setIsSaved,
    } = usePlayerStore();
    ...
    // Polling toutes les 30s au lieu de 5s (réduction de 83% des appels)
    useEffect(() => {
        if (!token) return;
        fetchState(token);
        ...
        }, 30000); // 30s polling ← was 5s
        return () => clearInterval(poll);
    }, [token, fetchState]);

    // Simulation de progression locale (1s tick)
    useEffect(() => {
        if (progressIntervalRef.current) clearInterval(progressIntervalRef.current);
        if (playbackState?.is_playing && !isSeeking) {
            progressIntervalRef.current = setInterval(() => {
                setProgressMs(Math.min(
                    usePlayerStore.getState().progressMs + 1000,
                    playbackState?.item?.duration_ms ?? 0
                ));
            }, 1000);
        }
        return () => { if (progressIntervalRef.current) clearInterval(progressIntervalRef.current); };
    }, [playbackState?.is_playing, playbackState?.item?.id, isSeeking, setProgressMs]);
    ...
    useEffect(() => {
        ...
        likeBatcher.setToken(tokenRef.current);
        likeBatcher.check(playbackState.item.id)
            .then(setIsSaved)
            .catch(() => setIsSaved(false));
    }, [playbackState?.item?.id, setIsSaved]);
    ...
    const handleSeekEnd = async () => {
        ...
        handleSeek(tokenRef.current, seekValueRef.current);
    };

    const handleVolumeChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!tokenRef.current) return;
        handleVolume(tokenRef.current, Number(e.target.value));
    };
            ...
            <AnimatePresence>
                {openPanel && token && (
                    ...
                    >
                        ...
                        {openPanel === 'devices' && (
                            <DevicesPanel token={token} activeDeviceId={activeDeviceId} onTransfer={() => fetchState(token)} />
                        )}
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <footer className="player-bar">
                <div className="player-bar__left">
                    ...
                    <Heart
                        ...
                        onClick={() => tokenRef.current && handleLike(tokenRef.current)}
                ...
                <div className="player-bar__center">
                    <div className="player-bar__controls">
                        <Shuffle size={16} color={shuffleOn ? '#1db954' : '#a7a7a7'} onClick={() => tokenRef.current && handleShuffle(tokenRef.current)} style={{ cursor: 'pointer' }} />
                        <SkipBack size={18} fill="#a7a7a7" color="#a7a7a7" onClick={() => tokenRef.current && handlePrevious(tokenRef.current)} style={{ cursor: 'pointer' }} />
                        <motion.button
                            whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}
                            onClick={() => tokenRef.current && handlePlayPause(tokenRef.current)}
                            className="player-bar__play-btn"
                        >
                            {isPlaying ? <Pause fill="black" size={18} /> : <Play fill="black" size={18} />}
                        </motion.button>
                        <SkipForward size={18} fill="#a7a7a7" color="#a7a7a7" onClick={() => tokenRef.current && handleNext(tokenRef.current)} style={{ cursor: 'pointer' }} />
                        <RepeatIcon size={16} color={repeatColor} onClick={() => tokenRef.current && handleRepeat(tokenRef.current)} style={{ cursor: 'pointer' }} />
                    </div>
                    <div className="player-bar__progress">
                        <span className="player-bar__time">{formatMs(progressMs)}</span>
                        <div className="player-bar__progress-track">
                            <div className="player-bar__progress-bg" />
                            <div
